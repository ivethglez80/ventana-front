          ...
          <table className="table-auto w-full">
            <thead>
              <tr>
                ...
                <th className="text-start pl-2">Compra</th>
                <th className="text-start pl-2">Nombre</th>
                <th className="text-start pl-2">Apellido</th>
                <th className="text-start pl-2">Fecha del Show</th>
              </tr>
            </thead>
            <tbody>
              {currentReservas.map((src, index) => (
                <tr
                  key={src.id}
                  className={`${
                    index % 2 === 0 ? "bg-blueD" : "bg-blueL"
                  } hover:text-blueB cursor-pointer relative`}
                >
                  ...
                  <td className="text-start pl-2 py-2 border-x-3 border-grayD">{src.compra}</td>
                  <td className="text-start pl-2 py-2 border-x-3 border-grayD">{src.nombre}</td>
                  <td className="text-start pl-2 py-2 border-x-3 border-grayD">{src.apellido}</td>
                  <td className="text-start pl-2 py-2 border-x-3 border-grayD">{src.fechaShow}</td>
                    ...
                    <td>
                      <div className="bg-grayD absolute left-8 mt-4 w-[420px] z-20 hover:text-grayL p-4 border border-grayL border-t-transparent">
                        <span className="flex flex-row border-b-2 border-grayL w-full h-full">
                          <p className="w-1/4">Show</p>
                          <p className="text-sm text-end w-full">CENA + SHOW DE TANGO Y FOLCLORE</p>
                        </span>

                        <span className="flex flex-row border-b-2 border-grayL w-full">
                          <p className="w-1/4">Descuento</p>
                          <p className="text-sm text-end w-full"></p>
                        </span>
                        <span className="flex flex-row border-b-2 border-grayL w-full">
                          <p className="w-1/4">Total</p>
                          <p className="text-sm text-end w-full">USD 520.00</p>
                        </span>
                        <span className="flex flex-row border-b-2 border-grayL w-full">
